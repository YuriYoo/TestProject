...
namespace SimpleAgent.Services
{
	class GPUStackClient
	{
		...
		public GPUStackClient()
		{
			_httpClient = new HttpClient { BaseAddress = new Uri(AppSettingsService.Settings.ApiBaseUrl) };
			// GPUStack 使用标准的 Bearer Token 进行鉴权
			_httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", AppSettingsService.Settings.ApiKey);
		}
		...
		public async Task<bool> CheckGPUStackIsStartAsync()
		{
			...
			catch (HttpRequestException httpEx)
			{
				...
				Trace.WriteLine($"[错误] 网络请求异常: {httpEx.Message}");
			}
			catch (Exception ex)
			{
				// 捕获其他未知错误
				Trace.WriteLine($"[错误] 未知的异常: {ex.Message}");
			}
		...
		{
			var targetModelName = AppSettingsService.Settings.ModelId;
			...
			try
			{
				...
				{
					Trace.WriteLine($"[警告] 请求失败: 状态码 {(int)response.StatusCode} ({response.StatusCode})");
					return false;
				}
			...
			{
				// 捕获 JSON 解析异常或其他未知错误
				Trace.WriteLine($"[错误] 解析或处理数据时发生异常: {ex.Message}");
			}
		...
		public async Task<GpuGlobalStatus> GetGlobalGpuStatusAsync()
		{
			...
			try
			{
				...
				if (!response.IsSuccessStatusCode)
				{
					Trace.WriteLine($"[警告] 请求硬件状态失败: {response.StatusCode}");
					return null;
				}
				...
				{
					Trace.WriteLine("未找到任何 GPU 设备信息。");
					return null;
				}
			...
			{
				Trace.WriteLine($"[错误] 获取或解析硬件数据时发生异常: {ex.Message}");
				return null;
			}
