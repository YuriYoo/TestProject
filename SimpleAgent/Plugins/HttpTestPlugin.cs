    ...
    public class HttpTestPlugin
    {
            ...
            [Description("自定义请求头，必须是合法的 JSON 对象字符串，例如 '{\"Authorization\": \"Bearer xxx\", \"Content-Type\": \"application/json\"}'。如无需要请留空")] string headersJson = "")
        {
            ...
            try
            {
                ...
                if (responseBody.Length > settings.Current.HttpTerminal)
                {
                    responseBody = string.Concat(responseBody.AsSpan(0, settings.Current.HttpTerminal), "\n...[内容过长，已被系统截断]...");
                }
            ...
            {
                return $"[错误] 请求超时 ({settings.Current.HttpTimeout}ms)。可能原因：服务未启动、端口被防火墙拦截，或者该接口发生死锁未返回任何数据。";
            }
