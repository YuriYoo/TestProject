...
namespace SimpleAgent.Plugins
{
	public class TerminalPlugin
	{
		...
		{
			return await BackgroundService.StartService(command, serviceId, _workingDirectory);
		}
			...
			[Description("需要执行的完整 CMD 命令，例如 'dotnet test' 或 'python script.py'")] string command)
		{
			...
			try
			{
				...
				catch (OperationCanceledException)
				{
					...
					process.Kill(true);
					return $"[错误] 命令运行超过了{AppSettingsService.Settings.TerminalTimeout}ms, 已自动终止。";
				}
				...
				return BackgroundService.TruncateByChars(sb.ToString(), AppSettingsService.Settings.TerminalTruncation);
			}
			catch (Exception ex)
			{
